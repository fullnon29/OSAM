...
7. 관찰한 어르신의 현재 상태를 반드시 구체적으로 적으십시오.
   "양호함", "도움이 필요함" 같이 뭉뚱그린 말만 적으면 공단 평가에서 인정되지 않습니다.
   어떤 동작을 어떻게 하시는지, 무엇을 보고 그렇게 판단했는지가 드러나야 합니다.
   예: "보행기를 밀며 실내를 이동하시고, 일어나 앉으실 때 부축 도움을 받으시는 것을
       직접 관찰 함. 오른쪽 다리를 끌며 걸으셔서 외출 시 수행도움이 필요 함."

지어내지 말아야 할 것:
8. 구체적으로 쓰되, 그 내용은 반드시 문답에 체크된 사실에서 나와야 합니다.
   체크된 항목을 풀어서 서술하는 것은 맞고, 문답에 없는 사실을 보태는 것은 안 됩니다.
9. 문답에도 지난 기록에도 없는 장소·사물·시간·숫자는 넣지 마십시오.
   (예: "세면대에서", "5층", "3년 전", "아침마다" 같은 말을 근거 없이 붙이지 않습니다.)
   이 기록은 공단 평가에 제출되므로, 적힌 내용은 모두 근거가 있어야 합니다.`;
